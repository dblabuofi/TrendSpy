angular.module("myApp").controller('queryMockController', function ($scope, $timeout, $filter, $uibModal, $interval, $http, $location, url, locationservice, $uibModal, $mdDialog) {
    $scope.querySelect = {
        value: ""
    };
    ...
    $scope.myFromDateValidator = function (fromDate, toDate, eventStart, eventEnd, eventType) {
...
//                console.log('here');
            ...
            var start = moment(eventStart, "MM-DD-YYYY");
//            console.log(start);
            if (from > start) {
                return "event date start date should after start date";
            }
            if (to < start) {
                return "event date start date should before end date";
            }
            if (eventType === 'pointEvent') {
                ...
                var end = moment(eventEnd, "MM-DD-YYYY");
                if (end < from) {
                    return "event end date should after start date";
                }
                if (end > to) {
                    return "event end date should before start date";
                }
...
//    console.log(isFirefox);
    ...
    $scope.submitLagTime = function () {
        ...
        $timeout(function(){
            clearWatch = $scope.$watch('silder', function (newVal, oldVal) {
    //        console.log('changed');
                $scope.data = [];
                var data = null;
    ...
    //            console.log(response.data);
        ...
        var finurl = url.url + "/GoogleTrendWS/googleMock?";
        console.log(finurl);
        ...
        param += "&count=" + $scope.template.count;
        param += "&eventFrom=" + encodeURIComponent(moment($scope.lagTimeQuery.eventStartDate, "MM-DD-YYYY").format("YYYY-MM-DD"));
        if ($scope.eventType.value === 'pointEvent') {
            param += "&eventTo=";
        } else {
            param += "&eventTo=" + encodeURIComponent(moment($scope.lagTimeQuery.eventEndDate, "MM-DD-YYYY").format("YYYY-MM-DD"));
        }
        ...
        $http({
            ...
            $timeout(function () {
                ...
                if (data.labels.length === 0) {
                    ...
                    $scope.labels = data.labels;
                    $scope.totalLabels = $scope.labels;
                    $scope.totalSeries = $scope.series = data.series.map(function (obj) {
                    ...
                    for (var i = 0; i < $scope.data.length; i += 2) {
                        $scope.hideData.push($scope.data[i]);
                        $scope.hideSeries.push($scope.series[i]);
                    }
...
//        $scope.constrastToDate = document.getElementById("ctoDate").value;
        ...
        var finurl = url.url + "/GoogleTrendWS/googleMock/contrast?";
        var param = "q=" + encodeURIComponent(keywords);
...
//        console.log(value);
        if (value == 'lagTime') {
            ...
            $scope.twitter.toDate = nowmoment.add(1, 'minutes').format("MM-DD-YYYY HH:mm:ss");
...
//        $scope.showTrendTime = false;
    ...
    $scope.hideTrendLine = function (showTrendTime) {
        ...
        if (showTrendTime === true && chartType == 'line') {
            ...
            $scope.series = $scope.totalSeries;
            $scope.colors = $scope.totalColors;
            $scope.datasetOverride = [
                {
                    type: 'line'
...
//        $scope.querySelect.value = 'contrastDifferentCity';
    ...
    var clearWatch = $scope.$watch('silder', function (newVal, oldVal) {
//        console.log('changed');
        $scope.data = [];
        var data = null;
        if ($scope.showTrendTime.value === false) {
...
//        var keywordsString = $scope.lagTimeQuery.keywords;
    ...
    $scope.lagtimeGenerate = function () {
        $scope.dateRangeShow = false;
        if ($scope.lagTimeQuery.ids == undefined)
            return;
        clearWatch();
        var ids = $scope.lagTimeQuery.ids.replace(/ /g, '');
        console.log('lagtimeGenerate');

        var finurl = url.url + "/GoogleTrendWS/googleMock/consistentlagtimes?";
        var param = "ids=" + encodeURIComponent(ids.replace(/ /g, ''));
        var arr = ids.split(",");
        var dates = "";
        for (var i in arr) {
            console.log(arr[i]);
            for (var j in $scope.full.data) {
                if ($scope.full.data[j][0] == arr[i]) {
                    dates += moment($scope.full.data[j][6], "MM-DD-YYYY HH:ss:mmZ").format("YYYY-MM-DD") + ",";
                }
            }
        }
        dates = dates.substring(0, dates.length - 1);
        param += "&dates=" + encodeURIComponent(dates);
        param += "&dateRange=" + encodeURIComponent($scope.lagTimeQuery.dayRange);
        param += "&windowRange=" + encodeURIComponent($scope.lagTimeQuery.windowRange);
        param += "&threshold=" + encodeURIComponent($scope.lagTimeQuery.threshold);
...
//
        $http({
            ...
            $timeout(function () {
                ...
                if (data.labels.length === 0) {
                    ...
                    $scope.totalSeries = $scope.series;
                    $scope.totalData = $scope.data;
                    $scope.hideData = [];
                    $scope.hideSeries = [];
                    for (var i = 0; i < $scope.data.length; i += 3) {
                        $scope.hideData.push($scope.data[i]);
                        $scope.hideSeries.push($scope.series[i]);
                    }
                    console.log($scope.hideData);
                    $scope.silder.min = 0;
                    $scope.silder.max = $scope.data[0].length - 1;
                    $scope.silder.lower = 0;
                    $scope.silder.upper = $scope.silder.max;
    ...
    $scope.lagTimeQuery.dayRange = 20;
    $scope.lagTimeQuery.ids = "1,2,3";
    $scope.lagTimeQuery.windowRange = 5;
    $scope.lagTimeQuery.threshold = 10;
